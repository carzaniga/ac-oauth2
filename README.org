...
This code provides some glue between the [[https://elpa.gnu.org/packages/oauth2.html][oauth2.el]] package and the
Gnus Emacs email client.  The goal is to have an authentication
process that is seamlessly integrated within Gnus and some of its
components used for fetching and sending messages, namely IMAP and
smtpmail.  In essence, this code uses functions from [[https://elpa.gnu.org/packages/oauth2.html][oauth2.el]] to
handle the mechanics of OAuth2 protocol, with some additional features
to automatically grab the authorization codes from the OAuth2
exchange, so that it can be used by IMAP and smtpmail.
